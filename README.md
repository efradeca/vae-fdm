...
2. **GNN encoder**: A message-passing neural network ([Gilmer et al., 2017](https://arxiv.org/abs/1704.01212)) that operates on graph connectivity. The same architecture and weight structure work on different mesh resolutions, though each model instance is built for a specific topology.

3. **Classical baseline**: Direct optimization of force densities via L-BFGS-B for neural vs classical comparison.

4. **Interactive explorer**: A standalone Qt + PyVista application that reproduces the real-time design workflow shown in the paper's Figure 14, without requiring Rhino3D ($995).
...
| L_physics | ~0 (< 1e-13) | 0.0 ± 0.0 | Table 1 |
...
The repository includes pretrained model weights in `data/` (~13MB total):

| Model | File | Task | Source |
|-------|------|------|--------|
| Formfinder (bezier) | `formfinder_bezier.eqx` | Shell | Paper weights (Pastrana et al.) |
| Formfinder (tower) | `formfinder_tower.eqx` | Tower | Paper weights (Pastrana et al.) |
| VAE (bezier) | `variational_formfinder_variational_bezier.eqx` | Shell | Our extension |
| VAE (tower) | `variational_formfinder_variational_tower.eqx` | Tower | Our extension (experimental) |

The deterministic models reproduce the paper's Table 1 results. The VAE models are our extensions for exploring solution diversity. Tower VAE is experimental: it produces diverse equilibrium solutions but with higher reconstruction error than the deterministic model due to the 656-dimensional latent space.
...
# Train the deterministic formfinder (paper baseline)
python train.py formfinder bezier          # ~3 min (shell)
python train.py formfinder tower           # ~15 min (tower)

# Train the variational formfinder (our extension)
python train.py variational_formfinder variational_bezier    # ~5 min
python train.py variational_formfinder variational_tower     # ~60 min

# Evaluate (paper Table 1)
python predict.py formfinder bezier --batch_size=100 --seed=90
python predict.py formfinder tower --batch_size=100 --seed=90

# Launch the interactive explorer (supports both bezier and tower)
python interactive_designer.py
```

## Interactive Explorer

The explorer shows metrics with equation references for both shell (bezier) and tower tasks:
...
python benchmarks/verify_all.py                         # 17 verification checks (incl. tower)
python benchmarks/validation_suite.py                   # 14 validation checks (neural vs classical)
python benchmarks/reproduce_paper.py                    # Paper Table 1 reproduction (single seed)
python benchmarks/reproduce_paper.py --seeds "[42,91,137,256,1024]"   # Multi-seed protocol w/ bootstrap CI
python benchmarks/benchmark_neural_vs_classical.py      # Neural vs classical benchmark (CSV + paired bootstrap CI)
python benchmarks/benchmark_architectures.py            # MLP vs GNN comparison
python benchmarks/benchmark_gnn_mesh_sizes.py           # GNN across mesh resolutions
python benchmarks/benchmark_vae_diversity.py            # VAE diversity quantification (bezier, CSV export)
python benchmarks/reproduce_paper_tower.py              # Tower Table 1 reproduction
python benchmarks/benchmark_tower_vae_diversity.py      # Tower VAE diversity
```

115 unit tests + 17 verification checks + 14 validation checks.
...
├── classical.py        # Classical baseline (direct optimization) [ours]
