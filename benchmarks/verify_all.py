...
4. Classical baseline (direct optimization)
...
def verify_classical():
    section_header("4. CLASSICAL BASELINE - Smoke Check")
    ...
    result = solve_classical(xyz, config, mesh, st, maxiter=2000, key=jrn.PRNGKey(42))

    check("Classical solver runs",
          result["l_shape"] > 0 and np.isfinite(result["l_shape"]),
          f"L_shape={result['l_shape']:.2f}")

    check("Classical L_physics near zero",
          result["l_physics"] < 1e-6,
          f"L_physics={result['l_physics']:.2e}")


# =============================================================================
# 5. TOWER DETERMINISTIC
# =============================================================================


def verify_tower():
    section_header("5. TOWER DETERMINISTIC")

    config_path = os.path.join(os.path.dirname(__file__), "..", "scripts", "tower.yml")
    model_path = os.path.join(DATA, "formfinder_tower.eqx")

    if not os.path.exists(model_path):
        check("Tower model exists", False, "formfinder_tower.eqx not found")
        return
    ...
    gen = build_data_generator(config)
    st = build_connectivity_structure_from_generator(config, gen)
    sk = build_neural_model("formfinder", config, gen, jrn.PRNGKey(90))
    mdl = load_model(model_path, sk)
    loss_fn_tower = build_loss_function(config, gen)

    xyz = gen(jrn.PRNGKey(0))
    x_hat = mdl(xyz, st)

    check("Tower forward pass finite",
          bool(jnp.all(jnp.isfinite(x_hat))),
          f"output shape: {x_hat.shape}")

    q = mdl.encode(xyz)
    has_tension = float(jnp.max(q)) > 0.01
    has_compression = float(jnp.min(q)) < -0.01
    check("Tower mixed tension/compression",
          has_tension and has_compression,
          f"q range: [{float(jnp.min(q)):.1f}, {float(jnp.max(q)):.1f}]")

    _, terms = loss_fn_tower(mdl, st, xyz[None, :], aux_data=True)
    check("Tower shape error reasonable",
          float(terms["shape error"]) < 5.0,
          f"shape error: {float(terms['shape error']):.2f} (paper: 1.4)")
