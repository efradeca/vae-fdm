...
def safe_stdev(x):
    return _stdev(x) if len(x) > 1 else 0.0
...
import yaml
from camera import CAMERA_CONFIG_BEZIER, CAMERA_CONFIG_TOWER
...
def predict_batch(
    ...
    _model_name = model_name.replace("_pinn", "")
    ...
    if time_batch_inference:
        ...
        print(f"Inference time on batch size {batch_size}: {mean(times):.5f} (+-{safe_stdev(times):.5f}) ms")
    ...
    print(f"Inference time over {num_predictions} samples (ms): {mean(opt_times):.4f} (+-{safe_stdev(opt_times):.4f})")

    labels = loss_terms_batch[0].keys()
    for label in labels:
        errors = [terms[label].item() for terms in loss_terms_batch]
        print(f"{label.capitalize()} over {num_predictions} samples: {mean(errors):.4f} (+-{safe_stdev(errors):.4f})")
