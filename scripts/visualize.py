...
import numpy as np
import yaml
from camera import CAMERA_CONFIG_BEZIER, CAMERA_CONFIG_BEZIER_TOP, CAMERA_CONFIG_TOWER
from compas.colors import Color, ColorMap
from compas.datastructures import mesh_transformed
from compas.geometry import Line, Polygon, Polyline, distance_point_point, length_vector
...
# ===============================================================================
    ...
    delta_min = 0.0
    delta_max = max(deltas_all, default=0.0)

    residual_min = 0.0
    residual_max = max(residuals_all, default=0.0)

    fmin = 0.0
    fmax = max(forces_all, default=0.0)

    fmin_comp = 0.0
    fmax_comp = max(forces_comp_all, default=0.0)

    fmin_tens = 0.0
    fmax_tens = max(forces_tens_all, default=0.0)
            ...
            if edgecolor == "force":
                ...
                for edge in mesh_hat.edges():
                    ...
                    if force == 0.0:
                        ...
                        _denom = max(_fmax - _fmin, 1e-12)
                        value = (fabs(force) - _fmin) / _denom
