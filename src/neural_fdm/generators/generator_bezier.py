...
from neural_fdm.generators.bezier import (
    BezierSurfaceAsymmetric,
    BezierSurfaceSymmetric,
    BezierSurfaceSymmetricDouble,
    evaluate_bezier_surface,
)
...
class BezierSurfacePointGenerator(PointGenerator):
    ...
    def __call__(self, key, wiggle=True):
        ...
        if wiggle:
            ...
            transform = jnp.zeros_like(self.surface.grid.tile)
...
class BezierSurfaceLerpPointGenerator(BezierSurfacePointGenerator):
    ...
    def __call__(self, key, wiggle=True):
        ...
        """
        if wiggle:
            key_this, key_other = jrn.split(key)
            transform_this = self.wiggle(key_this)
            transform_other = self.generator_other.wiggle(key_other)
        else:
            transform_this = jnp.zeros_like(self.surface.grid.tile)
            transform_other = jnp.zeros_like(self.generator_other.surface.grid.tile)
