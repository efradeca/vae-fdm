...
class PointGrid:
    ...
    def __init__(self, tile, num_pts) -> None:
        # The reindex map below is hard-coded for the 4x4 column-major
        # Rhino/Grasshopper convention used by the original paper. Other
        # grid sizes are not supported by this class (would corrupt
        # indexing silently).
        if num_pts != 4:
            raise ValueError(
                f"PointGrid currently only supports num_pts=4 (got {num_pts}). "
                "The hard-coded reindex map at self.indices assumes a 4x4 grid."
            )
        self.tile = tile
        self.num_pts = num_pts

        # Reindexing map: the 16 mirrored control points must be arranged in
        # the specific order expected by the Bezier surface evaluator (column-major,
        # matching the convention used in the original paper's Rhino/Grasshopper
        # pipeline). This ordering is fixed for num_pts=4.
