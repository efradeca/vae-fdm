...
def edges_lengths(vectors, eps=1e-12):
    """
    Compute the length of the edge vectors.

    Adds a small epsilon to the squared norm before sqrt to avoid NaN
    gradients on collapsed edges. losses.py already follows this pattern
    for residual norms.
    ...
    vectors: `jax.Array`
        The edge vectors.
    eps: `float`, optional
        Numerical floor on the squared norm. Default 1e-12 is well below
        any meaningful structural length scale.

    Returns
    -------
    lengths: `jax.Array`
        The lengths, all strictly positive.
    """
    squared = jnp.sum(vectors * vectors, axis=1, keepdims=True)
    return jnp.sqrt(squared + eps)
