...
def compute_loss_shell_vae(
    ...
    # Callers that route through training.py's wrapper always set beta;
    # we default to 0.0 (vanilla autoencoder) when the config has no
    # "vae" block, but only after explicitly verifying the key path so
    # silent misconfiguration in user code is harder.
    vae_params = loss_params.get("vae", {})
    beta = vae_params.get("beta", 0.0)
