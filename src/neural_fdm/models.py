...
class AutoEncoder(eqx.Module):
    ...
    def encode(self, x, structure=None):
        """
        ...
        x: `jax.Array`
            The target shape.
        structure: `EquilibriumMeshStructure`, optional
            The mesh structure. Required when the encoder is a `GNNEncoder`
            (it needs edge connectivity); ignored by MLP encoders.
        ...
        from neural_fdm.gnn import GNNEncoder
        if isinstance(self.encoder, GNNEncoder):
            return self.encoder(x, structure=structure)
