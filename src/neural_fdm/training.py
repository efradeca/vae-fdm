...
def train_step_vae(model, structure, optimizer, generator, opt_state, *,
    ...
    # IMPORTANT: build a fresh nested dict on every call so that the traced
    # `beta` value does not leak into the `loss_fn` partial's keywords, which
    # would be a persistent Python object shared across JIT boundaries.
    def vae_loss_wrapper(model, structure, x, aux_data=True):
        from neural_fdm.losses import compute_loss_vae
        _outer_params = loss_fn.keywords.get("loss_params", {})
        _vae_params = dict(_outer_params.get("vae", {}))
        _vae_params["beta"] = beta
        _loss_params = {**_outer_params, "vae": _vae_params}
