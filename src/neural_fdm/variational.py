...
def compute_diversity_metrics(
    ...
    """
    # Vectorized pure-JAX pairwise distances. A nested Python loop with
    # per-pair float() casts would force CPU evaluation and break
    # autodiff. The vectorized form keeps everything traceable and is
    # O(S^2) tensor work instead of S^2 Python iterations.
    S = x_hats.shape[0]
    x_diff = x_hats[:, None, :] - x_hats[None, :, :]   # (S, S, N3)
    q_diff = qs[:, None, :] - qs[None, :, :]           # (S, S, E)
    shape_pairs = jnp.sum(jnp.abs(x_diff), axis=-1)    # (S, S)
    q_pairs = jnp.sum(jnp.abs(q_diff), axis=-1)        # (S, S)
    # Upper-triangular (i < j) mask -- shape (S, S)
    iu = jnp.triu_indices(S, k=1)
    shape_dists = jnp.asarray(shape_pairs[iu])
    q_dists = jnp.asarray(q_pairs[iu])

    # Per-edge q standard deviation (where is there freedom?)
    q_std = jnp.std(qs, axis=0)

    # Per-node shape magnitude standard deviation
    x_reshaped = x_hats.reshape(S, -1, 3)
    shape_std = jnp.std(jnp.linalg.norm(x_reshaped, axis=-1), axis=0)

    import numpy as np  # only for the dict cast below; the math above is JAX

    has_pairs = shape_dists.shape[0] > 0
    return {
        "n_samples": int(S),
        "shape_pairwise_L1_mean": float(jnp.mean(shape_dists)) if has_pairs else 0.0,
        "shape_pairwise_L1_std": float(jnp.std(shape_dists)) if has_pairs else 0.0,
        "q_pairwise_L1_mean": float(jnp.mean(q_dists)) if has_pairs else 0.0,
        "q_std_per_edge": np.asarray(q_std),
        "q_std_mean": float(jnp.mean(q_std)),
        "shape_std_per_node": np.asarray(shape_std),
    }
